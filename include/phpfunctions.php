...
{
	$value = "";
				if($table=="cc_titulos" && $field=="anexo_titulo")
	{
		;
		return $value;
	}
				if($table=="cc_plan_pago" && $field=="anexo_confirmado")
	{
		;
		return $value;
	}
				if($table=="rj_master_anexos" && $field=="anexo_file")
	{
		;
		return $value;
	}
				if($table=="rj_master_provision_contable" && $field=="pc_anexo")
	{
		;
		return $value;
	}
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="cc_coactivo_anexos" && $field=="anexo_user")
	{
		return $_SESSION["UserID"];
	}
				if($table=="cc_acuerdo_pago" && $field=="sys_user")
	{
		return $_SESSION["UserID"];
	}
				if($table=="cc_actuaciones" && $field=="sys_user")
	{
		return $_SESSION["UserID"];
	}
				if($table=="cc_plan_pago" && $field=="sys_user")
	{
		return $_SESSION["UserID"];
	}
				if($table=="rj_master_audiencias" && $field=="sys_user")
	{
		return $_SESSION["UserID"];
	}
				if($table=="rj_master_anexos" && $field=="anexo_user")
	{
		return $_SESSION["UserID"];
	}
				if($table=="sc_master" && $field=="recepcion_fecha")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="sc_master" && $field=="fecha_control")
	{
		return date('%Y-%m-%d',strtotime('+30 days',strtotime(now())));
	}
				if($table=="sc_master" && $field=="sys_user")
	{
		return $_SESSION["UserID"];
	}
				if($table=="rj_master_provision_contable" && $field=="sys_user")
	{
		return $_SESSION["UserID"];
	}
	return "";
